...
//Defining the function for difference of matrces
double **linalg_sum3(double **a, double **b, double **d, int m, int n)
{
int i, j;
double **c;
c = createMat(m,n);
...
//End function for difference of matricesi
...
double **mult_int(float scalar,double **a,int m, int n)
{
  int i, j;
  for(i=0;i<m;i++)
  {
...
void save_E(double **a, int rows, int cols) {
  FILE *fp;
  fp = fopen("E.dat", "w");
...
void save_F(double **a, int rows, int cols) {
  FILE *fp;
  fp = fopen("F.dat", "w");
  //fprintf(fp, "The result of the multiplication is:\n");
  int i, j;
